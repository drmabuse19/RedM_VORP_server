...
Config.SearchRewardCount = {1,4} -- How many crawfish players can find per hole; Set this to a table like so {min,max} for random reward count per search; eg {0,3} will mean a random reward count between 0 and 3.

Config.CrawfishItemName = "fish" -- The item name of a Crawfish; given to player on successful search (MUST be registered in your items database!).
...
Config.CrawfishGivenItemName = "consumable_crowfish" -- The item name of what is rewarded when using a Crawfish from inventory (MUST be registered in your items database!).
...
Config.CrawfishGivenItemAmount = 4 -- How many items (Config.CrawfishGivenItemName) players can obtain when using a Crawfish from inventory; Set this to a table like so {min,max} for random item count when used; eg {1,3} will mean a random item count between 1 and 3.
